...
const updateAccount = async (req, res) => {
  try {
    ...
    const { nombre, apellido, email, username, direccion, rol, password} = req.body;
    console.log("Request body:", { nombre, apellido, email, username, direccion, rol });
    ...
    if (password) {
      account.password =  await userModel.encryptPassword(password); 
    }

    if (fullUrl) {
      account.imagen = fullUrl;
    }

    console.log("Updating user information in the database");
    const user = await userModel.findByIdAndUpdate(_id, account, { new: true });
    console.log("User updated successfully:", user);
    res.status(200).json(user);
  } catch (error) {
    console.error("Error in updateUser function:", error.message);
